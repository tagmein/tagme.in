...
function getChannelMetadataKey(channel: string): string {
...
export const onRequestGet: PagesFunction<Env> = async (context) => {
    try {
        ...
        let metadata = await context.env.TAGMEIN_KV.get(key, 'json')
        ...
        console.error('Error handling channel info request:', error)
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
    try {
        ...
        if (!channel || !metadata) {
            return new Response(JSON.stringify({ error: 'Channel and metadata are required' }), {
                status: 400,
                headers: { 'Content-Type': 'application/json' }
            })
        }
        ...
        const key = getChannelMetadataKey(channel)
        await context.env.TAGMEIN_KV.put(key, JSON.stringify(validMetadata))

        return new Response(JSON.stringify({ success: true, metadata: validMetadata }), {
            headers: { 'Content-Type': 'application/json' }
        })
    } catch (error) {
        console.error('Error handling channel info update:', error)
        return new Response(JSON.stringify({ error: 'Internal server error' }), {
            status: 500,
            headers: { 'Content-Type': 'application/json' }
        })
