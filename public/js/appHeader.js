const lightDarkModeButton = elem({
  attributes: {
    'data-tour': 'Switch between light and dark mode.',
...
const mainToolbar = elem({
  ...
  children: [
    elem({
      ...
      children: [
        elem({
          classes: ['icon', 'icon-home', 'display-on-channel'],
          tagName: 'span',
        }),
        elem({
...
const activityFilterInput = elem({
  ...
  events: {
    blur() {
      ...
      applyActivityFilter(activityFilterInput.value.trim());
    },
    focus() {
      lastKnownActivityFilterInput = activityFilterInput.value;
      activityFilterInputFocused = true;
      autocompleteActivitySearch.open();
    },
    input() {
      autocompleteActivitySearch.filter(activityFilterInput.value.trim());
    },
    keydown({ key }) {
      if (key === 'Enter') {
...
const autocompleteActivitySearch = displayAutocompleteActivitySearch(
  activityFilterInput,
...
const themeSelectorButton = elem({
  ...
  events: {
    click(e) {
      e.stopPropagation();
      if (document.body.getAttribute('data-mode') === 'theme-selector') {
        exitThemeSelector();
      } else {
        enterThemeSelector();
      }
  ...
  children: [
    elem({
      ...
      children: [
        ...
        elem({
          attributes: {
            'data-tour': 'Switch between public and private realms.',
          },
