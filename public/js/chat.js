...
How can I help you with this?`;
    ...
    toggleMainMenu() {
        const existingMenu = document.querySelector('.main-menu-container');
        if (existingMenu) {
            existingMenu.remove();
            return;
        }
        ...
        const menuItems = [
            { icon: '📋', text: 'Tasks', action: () => this.showTasksList() },
            { icon: '📅', text: 'Schedule', action: () => this.showDailySchedule() },
            { icon: '🔔', text: 'Reminders', action: () => this.showReminders() },
            { icon: '📊', text: 'Reports', action: () => this.showReports() }
        ];
    ...
    async sendMessage(context) {
        const messageText = context.message || this.inputArea.querySelector('.message-input').value.trim();
        if (!messageText) return;

        const timestamp = new Date().toISOString();
        // Use HOME_CHANNEL instead of 'default'
        const channel = context.channel || this.currentChannel || this.HOME_CHANNEL;
        ...
        const uiMessage = {
            ...
            timestamp: timestamp,
            channel: channel,
            messageId: context.messageId || this.currentMessageId
        };
        this.addMessage(uiMessage.text, uiMessage.sender);
        ...
        try {
            ...
            if (aiData && aiData.message) {
                this.addMessage(aiData.message, aiData.sender || 'Tagmein AI');
                if (aiData.suggestedReplies) {
                    this.displaySuggestedContent(aiData.suggestedReplies);
                }
            }
    ...
    addChatButtonsToChannels() {
        ...
        channels.forEach(channel => {
            ...
            chatButton.onclick = (e) => {
                e.stopPropagation();
                let channelName = channel.dataset.channel || channel.textContent.trim();
        ...
        if (menu) {
            ...
            const menuItems = [
                ...
                { text: 'Delete Chat', action: () => {
                    ...
                    if (this.currentChat && chatHistory[this.currentChat]) {
