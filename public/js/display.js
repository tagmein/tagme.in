...
function displayActivity() {
 ...
 async function scrollToDate(targetDate) {
  ...
  let attempts = 0
  const MAX_LOAD_ATTEMPTS = 20 // Increased to allow more loading attempts
  ...
  function checkForDateMatch() {
     ...
     if (dateStr) {
      try {
       ...
       if (!isNaN(itemDate.getTime())) {
         // Track earliest and latest dates for better error messages
         if (!earliestDateFound || itemDate < earliestDateFound) {
           earliestDateFound = itemDate;
         }
         if (!latestDateFound || itemDate > latestDateFound) {
           latestDateFound = itemDate;
         }
         
         // Log each date we're checking for debugging
         allDates.push({
          dateStr: dateStr,
          itemDate: itemDate,
         ...
         if (itemDate <= targetDateStart && 
             (!closestDate || itemDate > closestDate)) {
   ...
   console.log('Earliest date found:', earliestDateFound);
   console.log('Latest date found:', latestDateFound);
   
   // If we found a matching item, scroll to it
   if (closestItem) {
    foundMatchingItem = true
    console.log('Found matching item, scrolling to it');
    ...
    const formattedFoundDate = closestDate.toLocaleDateString(undefined, {
      year: 'numeric',
      month: 'long',
      day: 'numeric'
    });
    
    const formattedTargetDate = targetDateStart.toLocaleDateString(undefined, {
      year: 'numeric',
      month: 'long',
      day: 'numeric'
    });
    ...
    const notification = elem({
     ...
     textContent: closestDate.getTime() === targetDateStart.getTime() ?
       `Found activity from ${formattedFoundDate}.` :
       `Found activity from ${formattedFoundDate} (closest to ${formattedTargetDate}).`,
    })
  ...
  console.log('No matching date found in current content, loading more...');
  
  // If not found, load more content and check again
  while (!foundMatchingItem && attempts < MAX_LOAD_ATTEMPTS) {
   attempts++
   console.log(`Loading attempt ${attempts}/${MAX_LOAD_ATTEMPTS}`);
   ...
   try {
    ...
    console.log('loadMore result:', nextChunkResult);
    ...
    if (nextChunkResult === undefined || nextChunkResult < 0) {
     console.log('Reached end of content, breaking loop');
     break
    }
    
    // Check if we now have a matching item
    if (checkForDateMatch()) {
     console.log('Found matching date after loading more content');
     break
    }
   } catch (error) {
  ...
  if (!foundMatchingItem) {
   console.log('Could not find any matching date after multiple attempts');
   ...
   if (newsItems.length === 0) {
    ...
    // Target date is before earliest available date
    const formattedEarliestDate = earliestDate.toLocaleDateString(undefined, {
      year: 'numeric',
      month: 'long',
      day: 'numeric'
    });
    ...
    notificationText = `No activity found on or before ${formattedTargetDate}. Please try a different date.`;
   }
